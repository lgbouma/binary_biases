...
Let the fraction of detected planets from primaries at a given apparent radius 
be $F_1(r\a)$.
In the planet-less secondaries case, one can then see by analyzing 
Eqs~\ref{eq:n_det_0} and~\ref{eq:n_det_1} semi-analytically that 
$F_1(2r_\oplus)/F_1(2r_\oplus/\sqrt{2}) \approx 1.19$.
...
declines at large radii, and flattens below some cutoff $r_c$.
Thinking only about dilution from primaries is simplest:
at apparent radii $r\a < r_c/\sqrt{2}$, relatively more planets will be 
...
The detection efficiencies of real transit surveys are smooth functions in 
SNR; see for example the injection \& recovery simulations performed on {\it 
    Kepler}\ data by~\citet{christiansen_measuring_2016}, and the resulting 
smooth 
functional form adopted by~\citet{fulton_california-_2017}.
...
${\rm SNR} \approx 12$ in \citet{fulton_california-_2017}'s Fig 5.
To a good approximation, this allows one to discuss stars that are 
``searchable'' and ``not searchable'' for signals of an observed depth (at 
fixed orbital period).
For {\it Kepler} specifically, coupled with high resolution imaging of a
comparison sample of non-KOI host stars, this would facilitate the process 
of directly comparing the observed KOI distribution of apparent radii with the 
distributions discussed in this study.
...
\end{itemize} 
where the absolute normalization of the rates are less than those typical for 
Sun-like stars at least in part because of Robo-AO's sensitivity 
(see~\citealt{ziegler_robo-ao_2017}'s Fig.~2).
...
Although the data do not support as large of an uptick in companion rate 
as our Fig.~\ref{fig:frac_model_3} suggests, going from Neptunes to Earths,
there is still a weak increase in the detected planet companion fraction.
