...
from the source, i.e., $\sigma \propto 1/\sqrt{F}$,
...
\begin{figure}[!tb]
    ...
    star, one type of planet (with radius $r$), and
    $n\b/n\s=0.05$, for $n\b$ the number density of binary
    systems, and $n\s$ the number density of single stars.
    The occurrence
    rate of planets with radius $r$ is underestimated, and the
    occurrence rate of planets with radius
    $r/\sqrt{2}$ is overestimated.}
...
introduce the occurrence rate density $\Gamma$,
the number of planets per star per unit planet radius, so that
\begin{equation}
...
\end{equation}
where $\hat{\delta}$ is the Dirac delta function, and $\Lambda_{r\p}$ is the
number of planets with size $r\p$ per star considered in 
...
\end{align}
%The \hyperref[sec:appendix]{appendix} gives a more formal derivation
%of this equation.
...
\begin{align}
    ...
    \left[
       ...
       +
       {\mathcal{D}_2}\cdot\Gamma_2\left(\mathcal{D}_2r\a\right)
       \cdot
       {R(qM) \over R(M)}q^{-1/3}
       \right\rangle
    \right],
...
\texttt{4e517fe}.} Figure~\ref{fig:occ_rate_model_3_log} shows the
...
star, or a secondary star.  
This information can be used to help interpret the results of
a transit survey.  It is
...
different choices for $Z_2/Z_0$.  
Dividing each component of Equation~\ref{eq:general_Gamma_a} by
the total rate density, we get the fraction of
detections from single stars, primaries, and secondaries as a function
of apparent radius.
...
Here we present a more formal derivation of
...
\begin{equation}
    ...
    = \int_0^1 
      \mathrm{d}q\,f(q)\,\mathcal{N}_{\star, q}^1(r\a, M\a, q; r, M),
    \label{eq:fancyN_s_1}
\end{equation}
...
\begin{equation}
    \mathcal{N}_{\star,q}^1(r\a, M\a, q; r, M)
    \propto
    \hat{\delta}(M\a-M).
\end{equation}
In this case, $\mathcal{N}_{\star,q}^1$ is non-zero only at
...
\end{equation}
The normalization of $\mathcal{N}_{\star,q}^1$ is given by the
...
Applying the preceding arguments, we find
\begin{align}
    \mathcal{N}_{\star, q}^1(r\a, M\a, q; r, M)
    &=N_\star^0(r\a, M\a)
...
\begin{equation}
    \mathcal{N}_{\star,q}^2(r\a, M\a, q; r, M)
...
\begin{equation}
    ...
    =\int_0^1 \mathrm{d}q~f(q)\,\mathcal{N}_{\star, q}^2(r\a, M\a, q; r, M),
\end{equation}
where
\begin{align}
    \mathcal{N}_{\star,q}^2(r\a, M\a, q; r, M)
