...
We leave consideration of such biases for future work.}

\added{
It seems likely that to consistently account for binarity, one
must use a population inference model with parameters
controlling both the planet and binary star populations.
Given the difficulty of writing a correct likelihood function
for such a model, one feasible computational approach might be to
...
The NASA {\it TESS} mission will likely open up this avenue of research,
since its target stars are close and bright.
}
