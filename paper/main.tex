...
% Accounting for each effect requires some care.
...
single stars.
%relative difference $\Delta \Gamma_0 = |\Gamma_0~-~\Gamma\a|/\Gamma_0$
Fortuitously, the exponent of the radius distribution is close to
$-3$, and the errors from neglecting binaries almost entirely
cancel out.
...
\begin{equation}
    \Gamma_i(r,M) = Z_i \times \frac{r^\gamma}{\mathcal{N}_r} \times
...
\begin{align}
    ...
    \frac{1}{1+\mu}
    \left[
      1 + \frac{1}{\mathcal{N}_q} \frac{n\b}{n\s}\times 
    \left(
...
hosts the detected planet.  Intuitively, one would expect that the
primary star would be the more likely host because at fixed
planet size, planetary transits around secondaries have smaller
amplitudes (Equations~\ref{eq:dil_1} and~\ref{eq:dil_2} imply
$\mathcal{D}_2>\mathcal{D}_1$, if $\alpha=3.5$).  However, this
reasoning is qualitative, and its assumptions might not always
hold.
...
planets with binary companions increases by anywhere from $6\%$ to
$12\%$, depending on the relative occurrence of planets about
...
%This effect will appear in any radius distribution that declines at
%large radii, and flattens below some cutoff radius, $r_{\rm c}$.  This
%is because (for dilution about primaries) at apparent radii $r\a <
%r_{\rm c}/\sqrt{2}$, relatively more planets will be ``diluted'' into
%the given $r\a$, since there are more planets in the undiluted
%distribution from $r\a$ to $\sqrt{2}r\a$.
...
straight-forward, but also more tedious, method than that presented in
...
\begin{equation}
    \delta
    = \left[{r\over R(M\a)}\right]^2\times {L(M\a) \over L_{\rm tot}(M\a, q)}
    \equiv \left[{r\over R(M\a)}\right]^2\times 
...
\begin{equation}
      \delta =
      \left[{r\over R(qM\a)}\right]^2 \times
      {L(qM\a) \over L_{\rm tot}(M\a, q)}
      \equiv
      \left[{r\over R(M\a)}\right]^2\times
...
\begin{align}
    \mathcal{N}_{{\rm s},q}^2(r\a, M\a, q; r, M)
    =N_\star^0(\delta, L(M\a)) \cdot \frac{n\b}{n\s} 
    \cdot \mathcal{D}_1^3
...
\begin{align}
    ...
    &=N_\star^0(\delta, L(M\a))\cdot\Gamma_0(r\a, M\a) \cdot p_{\rm 
        tra}(M\a),
    \label{eq:n_det_0}
\end{align}
and
\begin{align}
    ...
    &=N_\star^0(\delta, L(M\a))\cdot p_{\rm tra}(M\a) \cdot
    \frac{n\b}{n\s}
    \int \mathrm{d}q \,
         \mathcal{D}_1^3 f(q)\cdot
         \mathcal{D}_1 \Gamma_1(\mathcal{D}_1r\a, M\a).
