...
\begin{align}
	\Lambda\a(r\a)
	={1 \over 1+\mu(\mathrm{BF})}
	\left[Z_0 \cdot \delta(r\a-r_0)+(Z_1+Z_2)\cdot \sqrt{2} 
	\mu(\mathrm{BF})\cdot 
...
\begin{align}
	...
	%&+\mathrm{BF}\cdot 
	%\int \mathrm{d}q\,f(q)
	%(1+q^\mathcal{A})^2 \cdot \Lambda^1\left( r\a\sqrt{1+q^\mathcal{A}}, 
	%M\a\right)\\
	%&+\mathrm{BF}\cdot
	%\int {\mathrm{d}q}\,f(q)q^{8/3}
	%\sqrt{1+q^{-\mathcal{A}}}(1+q^\mathcal{A})^{3/2}
	%\cdot\Lambda^2\left(r\a q\sqrt{1+q^{-\mathcal{A}}}, qM\a\right).
\end{align}
