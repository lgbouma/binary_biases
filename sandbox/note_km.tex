...
$\mathcal{N}(\pp\a, \ps\a; \pp, \ps)$, the number of searchable stars that 
give $(\pp\a, \ps\a)$ when the true system actually has $(\pp, \ps)$:
\begin{equation}
...
Imagine instead that we took $\Lambda_i(r,M) = Z_i f(r) = Z_i 
...
\begin{equation}
\Lambda_a(r_a) = \frac{r_a^\delta}{\mathcal{N}_r} \left[
\frac{Z_0}{1+\mu({\rm BF})}
+
...
\begin{equation}
\Lambda(r) = \frac{r^\delta}{\mathcal{N}_r} \left[
...
distribution) is a good unit test for the numerics. As of git commit 
\texttt{9b8f55c}, the two agree.
